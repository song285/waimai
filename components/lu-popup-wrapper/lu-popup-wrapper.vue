<template>
	<view :id="[popupId?'lupopupWrapper'+popupId:'']" class="lu-popup-wrapper" :class="popupClass" :style="[popupStyles]">
		<view class="lu-popup-content" :class="popupContentClass" :style="[{height:height,width:width,backgroundColor:backgroundColor,borderRadius:borderRadius}]" @touchmove.stop.prevent="">
			<slot></slot>
		</view>
		<view v-if="maskShow" class="lu-popup-mask" :class="popupMaskClass" @tap="close(maskClick)" @touchmove.stop.prevent=""></view>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			borderRadius:{
				type:String,
				default:"30rpx 30rpx 0 0"// left right top bottom center
			},
			type:{
				type:String,
